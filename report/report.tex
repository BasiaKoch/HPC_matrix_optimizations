...
performance by 2.53$\times$ relative to v5 at $n{=}8000$, and the final
version reaches 181.2~GFLOP/s at 76~threads.
\end{abstract}
...
implementation on CSD3~icelake, with a best measured throughput of
\textbf{181.2~GFLOP/s} at 76~threads for $n=8000$.
...
The v1$\to$v2 gain (0.26$\to$2.09~GFLOP/s, 8$\times$) is attributable mainly
to loop reordering and compiler flags.
The v5$\to$v6 gain (2.61$\to$7.21~GFLOP/s, 2.76$\times$) is consistent with
the low-level optimisations in v6 improving single-thread behaviour even in the
...
The lower reported parallel efficiency of v6 (38\%) compared to v5 (78\%)
should not be read as worse parallelism: v6 is faster than v5 at every tested
...
Figure~\ref{fig:headtohead} compares v5 and v6 at $n=8000$; v3 has no data at
this size (too slow at 1~thread to complete the full scaling study within the
time limit).
For context, at $n=4000$ and 76~threads --- the largest size at which v3
completed --- v6 achieves 205.0~GFLOP/s vs.\ v3's 65.7~GFLOP/s (3.12$\times$),
and v5 achieves 144.9~GFLOP/s (2.21$\times$ over v3).
...
Figure~\ref{fig:problemsize} shows that v6 throughput \emph{decreases} with $n$
at all tested thread counts.
At 76~threads, performance falls from 238.8~GFLOP/s at $n=2000$ to
...
v5 largely alleviates both effects by reducing the serial work to $O(\mathrm{NB}^3)$
...
The lower reported parallel efficiency of v6 (38\%) compared to v5 (78\%)
might suggest v6 parallelizes worse.
...
In these results, v6 is faster than v5 at every tested thread count (0.94~s
...
v6 achieves 238.8~GFLOP/s at 76T.
For large $n$ (8000, 512~MB), the working set far exceeds the combined node
L3 ($\approx$114~MB) and throughput falls to 181.2~GFLOP/s, consistent with
...
181~GFLOP/s at 76~threads for $n=8000$ on a single CSD3 icelake node,
...
achieving 59.4$\times$ speedup and 78.2\% parallel efficiency.
Additional locality, SIMD, and scheduling optimisations (v6) improve
