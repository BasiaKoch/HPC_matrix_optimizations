...
performance by 2.53$\times$ at $n{=}8000$, reaching 181.2~GFLOP/s at 76~threads.
\end{abstract}
...
per step ($\approx(n-p)^2/T$ FMAs) may fall below the cost of the barrier
...
2.47~GFLOP/s), consistent with improved locality in Phase~2 (OPT-2) and
better SIMD utilization in Phase~3 (OPT-3).
...
(512~MB) exceeds it by more than an order of magnitude, suggesting a shift
toward memory-bandwidth limitations at larger sizes.
...
likely reflects two effects: a serial work fraction that totals $O(n^2)$ across
all steps vs.\ $O(n^3)$ for the parallel trailing update, and the overhead of
$n=4000$ synchronisation points.
