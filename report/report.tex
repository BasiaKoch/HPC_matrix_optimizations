...
matrix (32~MB) lies near the 38~MB L3 boundary, whereas the $n=8000$ matrix
(512~MB) exceeds it by more than an order of magnitude, consistent with increasing memory-system pressure, including likely bandwidth
...
8~threads).
The full matrix (128~MB) exceeds the L3 cache (38~MB); as thread count grows,
...
For small $n$ (2000), the matrix ($n^2 \times 8 = 32$~MB) fits near L3 and the
computation appears more compute-bound; v6 achieves 238.8~GFLOP/s at 76T.
For large $n$ (8000, 512~MB), the working set far exceeds L3 and throughput
falls to 181.2~GFLOP/s, consistent with a shift toward memory-system bottlenecks.
